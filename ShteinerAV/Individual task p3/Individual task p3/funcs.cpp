...
void fill_company(DbCompanies& Db, const string& link) {
	...
	while (getline(file, str)) {
		...
		Db[current_comp].set_id(stoi(token));

		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp].set_name(token);

		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp++].set_address(token);
...
void fill_vacancy(DbVacancies& Db, const string& link) {
	...
	while (getline(file, str)) {
		...
		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp].set_v_id(stoi(token));

		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp].set_c_id(stoi(token));
		...
		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp].set_conditions(token);

		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp].set_requirements(token);

		getline(ss, token, ';');
		while (token[0] == ' ') token = token.substr(1);
		Db[current_comp++].set_salary(stoi(token));
	}
}
